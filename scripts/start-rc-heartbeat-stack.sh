...
usage() {
  local code="${1:-1}"
  printf 'Usage: %s [--hostname HOST] [--port PORT] [--model PROVIDER/MODEL] [--agent AGENT] [--prompt PROMPT] [--runtime-dir DIR]\n' "${0##*/}" >&2
  exit "$code"
}
...
if watcher_pid="$(read_pid_file "$watcher_pid_file" 2>/dev/null)"; then
  ...
  nohup env IMSG_BIN="$imsg_bin" OPENCODE_BIN="$opencode_bin" "$node_bin" "$watcher_script" --server-url "$server_url" --model "$model" --agent "$agent" --prompt "$prompt" >"$watcher_log_file" 2>&1 &
  watcher_pid="$!"
