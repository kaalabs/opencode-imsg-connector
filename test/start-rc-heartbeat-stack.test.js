...
test("start-rc-heartbeat-stack.sh starts the server and watcher and is idempotent", async (t) => {
  const tempDir = await makeTempDir("opencode-start-stack-")
  ...
  const cleanup = async () => {
    for (const fileName of ["opencode-server.pid", "rc-heartbeat-watcher.pid"]) {
      try {
  ...
  const first = await runCommand(
    "/bin/bash",
    [startStackPath, "--hostname", "127.0.0.1", "--port", port, "--runtime-dir", runtimeDir],
    { env },
  )

  assert.equal(first.code, 0)
  assert.match(first.stdout, /OpenCode server started at http:\/\/127\.0\.0\.1:/)
  assert.match(first.stdout, /RC heartbeat watcher started/)

  const firstServerPid = (await readFile(join(runtimeDir, "opencode-server.pid"), "utf8")).trim()
  const firstWatcherPid = (await readFile(join(runtimeDir, "rc-heartbeat-watcher.pid"), "utf8")).trim()
  ...
  assert.match(second.stdout, /RC heartbeat watcher already running/)

  const secondServerPid = (await readFile(join(runtimeDir, "opencode-server.pid"), "utf8")).trim()
  const secondWatcherPid = (await readFile(join(runtimeDir, "rc-heartbeat-watcher.pid"), "utf8")).trim()
